...
app.post('/api/ai-horoscope', checkPremium, async (req, res) => {
  ...
  'Lagna: '+(ascendant?.sign||'')+' | Moon Rashi: '+moonSign+' | Sun Sign: '+(chartData.sunSign||''),
  'Nakshatra: '+nakshatra+' Pada '+nakshatraPada+' (lord: '+nakshatraLord+', deity: '+(nakshatraDeity||'')+', quality: '+(nakshatraQuality||'')+')',
  'Moon sign emotional core: '+moonTheme,
  '',
  'CURRENT KARMIC PERIOD (Vimshottari Dasha):',
  dl+' Mahadasha → '+al+' Antardasha → '+pl+' Pratyantardasha (ends '+dasha?.endDate+')',
  'This means: '+dl+' is the ruling theme of life right now, '+al+' is shaping the current chapter, '+pl+' triggers specific events.',
  '',
  'PLANETARY STRENGTHS:',
  'Strong planets (>65%): '+(strongPlanets||'none')+' — these areas of life flourish',
  'Weak planets (<35%): '+(weakPlanets||'none')+' — these areas need work',
  '',
  'CRITICAL HOUSE LORDS:',
  '• 1st (self/life path): '+houses?.find(h=>h.house===1)?.lord,
  '• 2nd (wealth/family): '+houses?.find(h=>h.house===2)?.lord,
  '• 4th (home/mother/property): '+houses?.find(h=>h.house===4)?.lord,
  '• 5th (children/love/creativity): '+(h5?.lord||''),
  '• 7th (marriage/partnerships): '+(h7?.lord||''),
  '• 9th (fortune/dharma/father): '+houses?.find(h=>h.house===9)?.lord,
  '• 10th (career/status): '+(h10?.lord||''),
  '• 11th (gains/desires): '+houses?.find(h=>h.house===11)?.lord,
  '',
  'YOGAS (auspicious combinations): '+yogaList,
  'DOSHAS (challenging patterns): '+doshaList,
  'Sade Sati: '+(sadeSati?.active?'YES — '+sadeSati.phase+' phase. This is a 7.5 year transformative period.':'No'),
  '',
  'KEY DRISHTI (Aspects):',
  keyAspects,
  '',
  ...
  keyHouseLords,
  '',
  'CURRENT TRANSITS (in houses from natal lagna):',
  keyTransits,
  '',
  demoContext,
  '',
  'WRITING TASK:',
  'Write a complete, deeply specific Vedic astrological forecast for '+periodFocus,
  '',
  'CRITICAL RULES — every reading must follow these:',
  '1. SYNTHESIZE — never say "Mars affects career" alone. Say "Mars (your 10th lord) is currently in the 6th house from your lagna, transiting through Tula — this means workplace conflicts may peak around mid-month, but persistence will earn you recognition by month-end"',
  '2. BE SPECIFIC — mention exact houses, exact lords, exact aspects',
  '3. PROVIDE TIMING — use the Antardasha lord '+al+' and current transits to predict WHEN things happen',
  '4. ACTIONABLE — every section must end with what to DO, not just what will happen',
  '5. LIFE-STAGE AWARE — the user is '+(demo.ageGroup||'')+' '+(demo.lifeStage||'')+' '+(demo.relationshipStatus||'')+'. Tailor every prediction to their reality.',
  '6. INTEREST-FOCUSED — they want guidance on: '+(Array.isArray(demo.primaryInterest)?demo.primaryInterest.join(', '):demo.primaryInterest||'')+'. Give 50% more depth to these topics.',
  '',
  'STRUCTURE — use these EXACT headers and rules:',
  '',
  '## 🌌 The Cosmic Theme',
  '[6-7 sentences. Open with the dominant theme from '+dl+'-'+al+' Dasha. Reference Moon sign emotional nature. Mention Sade Sati if active. Set the scene like a wise elder guiding through this life chapter.]',
  '',
  '## 💼 Career & Wealth (Artha)',
  '[6-7 sentences. Reference 10th lord ('+(h10?.lord||'')+') current placement. Reference 11th lord (gains). Use '+dl+' Mahadasha context. Mention specific dates/days when career energy peaks. End with ONE specific action: a day to negotiate, a meeting to schedule, a project to delay.]',
  '',
  '## 💞 Relationships & Love (Kama)',
  '[6-7 sentences. Reference 7th lord ('+(h7?.lord||'')+') and Venus position. For singles — mention Dasha periods that bring meeting opportunities. For married — discuss harmony patterns from current transits. End with ONE specific action.]',
  '',
  '## 🌿 Health & Vitality (Arogya)',
  '[5-6 sentences. Reference 6th lord (illness) and 8th lord (longevity). Mention Moon sign Ayurvedic constitution. Identify specific dates of energy dips. End with ONE specific Ayurvedic or yoga practice.]',
  '',
  '## 🪔 Spiritual Growth (Dharma-Moksha)',
  '[5-6 sentences. Reference 9th lord (dharma) and 12th lord (moksha). Use '+nakshatra+' Nakshatra wisdom. Reference Nakshatra deity '+(nakshatraDeity||'')+'. End with ONE specific spiritual practice.]',
  '',
  '## 🙏 Vedic Remedies (Upaya)',
  '[3-4 specific remedies. Each must be tied to a specific planet/house issue identified above. Include: 1 mantra (Sanskrit), 1 lifestyle change, 1 charity/seva, 1 gemstone or yantra suggestion. Be specific to the planet causing issues.]',
  '',
      'STYLE RULES — follow these exactly:',
      '- Write like Susan Miller: warm, personal, encouraging, conversational. Never cold or generic.',
      '- NEVER mention any astrologer names, websites, or external references in the output',
